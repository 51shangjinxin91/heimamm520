  ...
  <div class="form-box">
    ...
    <el-form :rules="rules" class="login-form" ref="form" :model="form">
      ...
      <el-form-item>
        ...
        <el-button class="register-button" @click="dialogFormVisible = true" type="success">注册</el-button>
      </el-form-item>
    </el-form>
  </div>
  ...
  <el-dialog title="收货地址" :visible.sync="dialogFormVisible">
    <el-form :model="form">
      <el-form-item label="昵称" :label-width="formLabelWidth">
        <!-- 头像上传 -->
        <el-upload class="avatar-uploader" name='image' action=" https://jsonplaceholder.typicode.com/posts/" :show-file-list="false" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
  ...
  data() {
    ...
    var checkPhone = (rule, value, callback) => {
      if (!value) {
        ...
        // 判断是否符合
        // .test(验证的字符串) 返回的是true 或 false
        if (reg.test(value) == true) {
          callback();
        } else {
          // 不满足手机号的格式
          callback(new Error("老铁,你的手机号输错了"));
        }
      }
    };
    // 表单的数据
...
.login-box {
  ...
  display: flex;
  /* 上下居中 */
  align-items: center; // 左右均分
  justify-content: space-around; // 背景线性渐变
